              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select id="subcategory" v-model="form.subcategory"
                    ...
                    <option v-for="item in subCategories" :key="item.value" :value="item.id">
                      {{ item.name }}
                    </option>
                  </select>
                </div>
...
const subCategories = ref(props.categories?.[0]?.subcategory || [])
...
const formatTools = ['Bold', 'Italic', 'Underline', 'Quote', 'Code', 'List', 'Link', 'Full Screen', 'Image', 'YouTube']
...
const submitPost = async () => {
  ...
  try {
    ...
    if (!form.title || !form.content || !form.language ||
    ...
    try {
      ...
      const response = await $torrentApi.saveTorrent(formData);
...
const copyToClipboard = async (text) => {
  try {
    ...
    const urlObj = announceUrls.find(url => url.link === text)
    copiedUrl.value = urlObj.id
...
const handleCategoryChange = () => {
  const selected = categories.value.find(cat => cat.id === parseInt(form.category))
  subCategories.value = selected ? selected.subcategory : []
  form.subcategory = "" // reset subcategory
}
