  ...
  <AppLayout>
    ...
    <div v-if="torrent">
      ...
      <div class="bg-gray-700 px-4 py-2 gap-2 rounded-t flex justify-between">
        ...
        <h1 v-else class="text-lg font-semibold">{{ torrent.name }}</h1>
      ...
      <div class="bg-gray-800 rounded-b mb-4">
        <div class="grid lg:grid-cols-3 gap-2 p-4 text-sm">
          ...
          <div class="space-y-2">
            ...
            <!-- <a href="https://mostbeautuleedehad.com?d6K5T=1239604" target="_blank" rel="noreferrer"
              class="w-full bg-green-600 hover:bg-green-700 text-white py-2 px-4 rounded flex items-center justify-center space-x-2 font-semibold">
              <i class="flaticon-torrent-download"></i>
     ...
     <div class="flex justify-between mb-4">
        <!-- <AdPcBanner adKey="4b64dd26f703e598b00d70fae697a73b" :width="728" :height="90" />         -->
      </div>
        ...
        <div class="p-2">
          <div v-if="activeTab === 'DESCRIPTION'">
            ...
            <div v-if="!isLoggedIn && torrent?.content" class="relative">
              <div v-html="torrent.content" ref="contentContainer"
...
import AdPcBanner from '~/components/AdPcBanner.vue';
...
// fetch blog data from Laravel API (SSR) with caching
const { data: torrent, error } = await useFetch(
  `${config.public.apiBase}/torrent/get/${route.params.id}/${route.params.slug}`,
  {
    key: `torrent-${route.params.id}-${route.params.slug}`,
    // Server-side: use Nuxt's built-in request deduplication and caching
    server: true,
    // This ensures the cache is shared across all SSR requests (all browsers)
    // Nuxt will cache the response on the server and reuse it for subsequent requests
    default: () => null,
  }
)
...
if (torrent.value) {
  ...
  // Build canonical URL using the canonical slug from API (not the URL slug)
  const canonicalPath = `/torrent/${route.params.id}/${torrent.value.slugged_title}`
  const canonicalUrl = `https://1337x.digital${canonicalPath}`
...
const handleClick = () => {
  if (typeof window !== 'undefined' && typeof window.poClick === 'function') {
...
const goToDirectDownload = (torrent)=>{
  if(torrent.category_id == 5){
    window.open("https://mostbeautuleedehad.com?d6K5T=1239604","_blank")
  }
  else{
    window.open("https://mostbeautuleedehad.com?d6K5T=1239604","_blank")
  }
...
// Watch for torrent content changes to re-track images
watch(() => torrent.value?.content, () => {
