  ...
  <div class="bg-white rounded-lg shadow-md overflow-hidden">
    ...
    <div class="overflow-x-auto">
      <table class="w-full">
        ...
        <tbody>
          <tr v-for="torrent in torrents" :key="torrent.id" class="border-b hover:bg-gray-50 transition-colors">
            <td class="px-4 py-3">
              <div class="flex items-center">
                <span class="mr-2">🎬</span>
                <button aria-label="handle torrent" @click="handleTorrentClick(torrent)"
            ...
            <td class="px-4 py-3 text-center">
              <span class="px-2 py-1 rounded text-xs font-semibold" :class="getSeedersClass(torrent.seeders)">
                {{ torrent.seeders }}
              </span>
            </td>
            <td class="px-4 py-3 text-center">
              <span class="px-2 py-1 rounded text-xs font-semibold" :class="getLeechersClass(torrent.leechers)">
                {{ torrent.leechers }}
              </span>
            </td>
            ...
            <td class="px-4 py-3 text-center">
              <button aria-label="handle uploader" @click="handleUploaderClick(torrent.uploader)"
            ...
            <td class="px-4 py-3 text-center">
              <div class="flex justify-center space-x-2">
                <button aria-label="handle download" @click="handleDownload(torrent)"
                  class="text-green-600 hover:text-green-800 transition-colors" title="Download">
                  <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z" />
                  </svg>
                </button>
                <button aria-label="handle magnet" @click="handleMagnet(torrent)"
                  class="text-red-600 hover:text-red-800 transition-colors" title="Magnet Link">
                  <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    <path
                      d="M3,7V13A9,9 0 0,0 12,22A9,9 0 0,0 21,13V7H18V13A6,6 0 0,1 12,19A6,6 0 0,1 6,13V7H3M12,2A2,2 0 0,1 14,4A2,2 0 0,1 12,6A2,2 0 0,1 10,4A2,2 0 0,1 12,2Z" />
                  </svg>
                </button>
              </div>
            </td>
    ...
    <div v-if="showPagination" class="bg-gray-50 px-4 py-3 border-t">
      <div class="flex justify-between items-center">
        ...
        <div class="flex space-x-2">
          <button aria-label="handle previous" @click="previousPage" :disabled="currentPage === 1"
          ...
          <button aria-label="handle next" @click="nextPage" :disabled="currentPage === totalPages"
            class="px-3 py-1 bg-white border rounded hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
            Next
          </button>
        </div>
